...
resource "prismacloud_policy" "op_exp8_r12" {
   name = "Filtro de métricas y alarmas de AWS Log no existe para los cambios en las listas de control de acceso a la red (NACL)"
   policy_type = "config"
   cloud_type = "aws"
   severity = "informational"
   description = <<-EOT
Esta política identifica las regiones de AWS que no tienen un filtro de métricas de registro ni una alarma para cambios de VPC. Monitorear los cambios en VPC ayudará a garantizar que los recursos y servicios no queden expuestos involuntariamente. Se recomienda establecer un filtro métrico y una alarma para los cambios realizados en las VPC.
...
{ ($.eventName = CreateNetworkAcl) || ($.eventName = CreateNetworkAclEntry) || ($.eventName = EliminarNetworkAcl) || ($.eventName = EliminarNetworkAclEntry) || ($.eventName = ReemplazarNetworkAclEntry) || ($.eventName = ReemplazarNetworkAclAssociation) }
