resource "prismacloud_policy" "op_exp8_r4" {
   name = "Filtro de métricas y alarmas de AWS Log no existen para API no autorizadas"
   policy_type = "config"
   cloud_type = "aws"
   severity = "informational"
   description = <<-EOT
Esta política identifica las regiones de AWS que no tienen un filtro de métricas de registro ni alarmas para llamadas API no autorizadas. Monitorear las llamadas API no autorizadas ayudará a revelar errores de la aplicación y puede reducir el tiempo para detectar actividad maliciosa. Se recomienda establecer un filtro de métricas y una alarma para llamadas API no autorizadas.
...
{ ($.errorCode = "*Operación no autorizada") || ($.errorCode = "Acceso denegado*") }
