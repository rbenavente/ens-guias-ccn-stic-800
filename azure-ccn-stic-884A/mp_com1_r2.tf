resource "prismacloud_policy" "mp_com1_r2" {
   name = "Azure Network Security Group con regla de entrada demasiado permisiva para todo el tráfico en el protocolo TCP"
   policy_type = "config"
   cloud_type = "azure"
...
Esta política identifica que los grupos de seguridad no permitan todo el tráfico de Internet. Un grupo de seguridad actúa como un firewall virtual que controla el tráfico de una o más instancias. Los grupos de seguridad deben tener ACL restrictivas para permitir solo el tráfico entrante desde IP específicas a puertos específicos donde la aplicación está escuchando conexiones.
  EOT
   ...
   rule {
        name = "Azure Network Security Group con regla de entrada demasiado permisiva para todo el tráfico en el protocolo TCP"
        criteria = "config from cloud.resource where cloud.type = 'azure' AND api.name= 'azure-network-nsg-list' AND json.rule = securityRules[?any((sourceAddressPrefix equals Internet or sourceAddressPrefix equals * or sourceAddressPrefix equals 0.0.0.0/0 or sourceAddressPrefix equals ::/0) and protocol equals Tcp and access equals Allow and direction equals Inbound and destinationPortRange contains *)] exists"
        parameters = {
          savedSearch = false
          withIac     = false
        }
