resource "prismacloud_policy" "mp_com1_r4" {
   name = "Azure Network Security Group con regla de salida demasiado permisiva"
   policy_type = "config"
   cloud_type = "azure"
...
Esta política identifica que los grupos de seguridad no permitan todo el tráfico de Internet. Un grupo de seguridad actúa como un firewall virtual que controla el tráfico de una o más instancias. Los grupos de seguridad deben tener ACL restrictivas para permitir solo el tráfico entrante desde IP específicas a puertos específicos donde la aplicación está escuchando conexiones.
  EOT
   ...
   rule {
        name = "Azure Network Security Group con regla de salida demasiado permisiva"
        criteria = "config from cloud.resource where cloud.type = 'azure' AND api.name= 'azure-network-nsg-list' AND json.rule = securityRules[?any(access equals Allow and direction equals Outbound and (sourceAddressPrefix equals * or sourceAddressPrefix equals 0.0.0.0/0 or sourceAddressPrefix equals ::/0) and (destinationAddressPrefix equals * or destinationAddressPrefix equals Internet))] exists"
        parameters = {
          savedSearch = false
          withIac     = false
        }
