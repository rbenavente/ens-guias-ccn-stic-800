resource "prismacloud_policy" "op_acc2_r2" {
   name = "Puertos administrativos no permitidos sin JIT (RDP)"
   policy_type = "config"
   cloud_type = "azure"
   severity = "low"
   description = <<-EOT
