resource "prismacloud_policy" "op_exp11_r1" {
   name = "Acceso Internet Key Vault [Datos sensibles]: Acceso permitido al Key Vault desde Internet sin restricciones de red"
   policy_type = "config"
   cloud_type = "azure"
   severity = "low"
   description = <<-EOT
Azure Key Vault protege las claves criptográficas, los certificados (y las claves privadas asociadas a estos) y los secretos (como cadenas de conexión y contraseñas) en la nube. No obstante, cuando almacena datos confidenciales y críticos para la empresa, debe tomar medidas para maximizar la seguridad de los almacenes y de los datos almacenados en estos.
  EOT
   ...
   rule {
        name = "Acceso Internet Key Vault [Datos sensibles]: Acceso permitido al Key Vault desde Internet sin restricciones de red"
        criteria = "config from cloud.resource where api.name = 'azure-key-vault-list' AND json.rule = (tags.dataclassification equals \"secret\" or tags.dataclassification equals \"restricted\") and (properties.privateEndpointConnections does not exist or properties.privateEndpointConnections is empty) and ( properties.networkAcls does not exist or properties.networkAcls.defaultAction equals \"Allow\" )"
        parameters = {
          savedSearch = false
          withIac     = false
        }
