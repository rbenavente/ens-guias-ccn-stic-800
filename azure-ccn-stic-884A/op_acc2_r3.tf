resource "prismacloud_policy" "op_acc2_r3" {
   name = "Puertos administrativos no permitidos sin JIT (SSH)"
   policy_type = "config"
   cloud_type = "azure"
   severity = "low"
   description = <<-EOT
